...
pub fn fsmentry(item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let item = parse_macro_input!(item as FsmEntry);
    _dsl(item)
...
mod tests {
    ...
    fn example() {
        let file = syn::parse2(
            syn::parse_str::<FsmEntry>(include_str!("example.dsl"))
                .unwrap()
                .map_mermaid(|()| Mermaid::default())
                .to_token_stream(),
        )
        .unwrap();
        expect_file!["example.rs"].assert_eq(&prettyplease::unparse(&file));
    }
